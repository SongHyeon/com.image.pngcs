namespace Pngcs
{
    class PngDeinterlacer
    {
        ...
        readonly ImageInfo imi;
        int pass; // 1-7
        int rows, cols, dY, dX, oY, oX, oXsamples, dXsamples; // at current pass
        ...
        int currRowReal = -1;
        readonly int packedValsPerPixel;
        readonly int packedMask;
        readonly int packedShift;
        int[][] imageInt; // FULL image -only used for PngReader as temporary storage
        byte[][] imageByte;
        ...
        void deinterlaceIntPacked ( int[] src , int[] dst )
        {
        ...
        void deinterlacePackedByte ( byte[] src , byte[] dst )
        {
            int spos, smod, smask; // source byte position, bits to shift to left (01,2,3,4
            int tpos, tmod, p, d;
         ...
         */
        internal bool isAtLastRow () => pass==7 && currRowSubimg==rows-1;

        /**
         * current row number inside the "sub image"
         */
        internal int getCurrRowSubimg () => currRowSubimg;

        /**
         * current row number inside the "real image"
         */
        internal int getCurrRowReal () => currRowReal;

        /**
         * current pass number (1-7)
         */
        internal int getPass () => pass;
        ...
        internal int getPixelsToRead () => getCols();

        internal int[][] getImageInt () => imageInt;

        internal void setImageInt ( int[][] imageInt ) => this.imageInt = imageInt;

        internal byte[][] getImageByte () => imageByte;
