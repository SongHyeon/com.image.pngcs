namespace Pngcs {
    ...
    internal class PngIDatChunkInputStream : Stream {
        ...
        private readonly Pngcs.Zlib.CRC32 crcEngine;
        ...
        private void EndChunkGoForNext() {
            ...
            // Skips empty IDATS
            do {
                int crc = Pngcs.PngHelperInternal.ReadInt4(inputStream); //
                offset += 4;
                if (checkCrc) {
                ...
                ended = !PngCsUtils.arraysEqual4(idLastChunk, Pngcs.Chunks.ChunkHelper.b_IDAT);
                if (!ended) {
        ...
        public void ForceChunkEnd() {
            if (!ended) {
                ...
                Pngcs.PngHelperInternal.ReadBytes(inputStream, dummy, 0, toReadThisChunk);
                if (checkCrc)
