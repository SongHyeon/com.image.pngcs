namespace Pngcs
{
    ...
    internal class FilterWriteStrategy
    {
        ...
        static readonly int COMPUTE_STATS_EVERY_N_LINES = 8;

        readonly ImageInfo imgInfo;
        readonly FilterType configuredType; // can be negative (fin dout) 
        FilterType currentType; // 0-4 
        int lastRowTested = -1000000;
        double[] lastSums = new double[5];// performance of each filter (less is better) (can be negative)
        double[] lastEntropies = new double[5];
        double[] preference = new double[] { 1.1, 1.1, 1.1, 1.1, 1.2 }; // a priori preference (NONE SUB UP AVERAGE PAETH)
        int discoverEachLines = -1;
        ...
        internal FilterWriteStrategy ( ImageInfo imgInfo , FilterType configuredType )
        {
            ...
            {
                currentType = ( (imgInfo.Rows<8 && imgInfo.Cols<8) || imgInfo.Indexed || imgInfo.BitDepth<8 )
        ...
        internal void FillResultsForFilter ( int rown , FilterType type , double sum , int[] histo , bool tentative )
        {
            ...
            if( histo!=null )
            {
                ...
                for( int i=0 ; i<256 ; i++ )
                {
                    ...
                    if( tentative )
                        e += v>0.00000001 ? v * System.Math.Log(v) : 0.0;
                    else
                        histogram1[i] = v;
                }
