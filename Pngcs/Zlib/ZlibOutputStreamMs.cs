...
internal class ZlibOutputStreamMs : AZlibOutputStream
{
        ...
        public ZlibOutputStreamMs ( IO.Stream st, int compressLevel, EDeflateCompressStrategy strat, bool leaveOpen)
            : base( st , compressLevel , strat , leaveOpen )
        { 

        }

        IO.Compression.DeflateStream deflateStream; // lazily created, if real read/write is called
        ...
        void initStream ()
        {
            ...
            deflateStream = new IO.Compression.DeflateStream( rawStream , clevel , true );
        }
