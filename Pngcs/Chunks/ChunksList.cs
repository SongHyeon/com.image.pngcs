...
{ 
    ...
    public class ChunksList
    {
        ...
        /// <summary> Includes all chunks, but IDAT is a single pseudo chunk without data </summary>
        ...
        /// <summary> Keys of processed (read or writen) chunks </summary>
        ...
        /// <summary> Returns a copy of the chunk list (but the chunks are not copied) </summary>
        /// <remarks> This should not be used for general metadata handling </remarks>
        public List<PngChunk> GetChunks () => new List<PngChunk>(chunks);

        internal static List<PngChunk> GetXById ( List<PngChunk> list , string id , string innerid )
        {
            if( innerid==null ) return ChunkHelper.FilterList( list , new ChunkPredicateId(id) );
            else return ChunkHelper.FilterList( list , new ChunkPredicateId2(id,innerid) );
        }

        /// <summary> Adds chunk in next position. This is used only by the pngReader </summary>
        ...
        /// <summary> All chunks with this ID </summary>
        /// <remarks>The GetBy... methods never include queued chunks</remarks>
        /// <returns>List, empty if none</returns>
        public List<PngChunk> GetById ( string id ) => GetById(id, null);

        /// <summary> Same as ID, but we an additional discriminator for textual keys </summary>
        /// <remarks>If innerid!=null and the chunk is PngChunkTextVar or PngChunkSPLT, it's filtered by that id</remarks>
        /// <returns>List, empty if none</returns>
        public List<PngChunk> GetById ( string id , string innerid ) => GetXById( chunks , id , innerid );

        /// <summary> Returns only one chunk  </summary>
        /// <returns>First chunk found, null if not found</returns>
        public PngChunk GetById1 ( string id ) => GetById1( id , false );

        /// <summary> Returns only one chunk </summary>
        /// <param name="failIfMultiple">true, and more than one found: exception</param>
        /// <returns>null if not found</returns>
        public PngChunk GetById1 ( string id , bool failIfMultiple ) => GetById1( id , null , failIfMultiple );

        /// <summary> Sames as <c>GetById1(string id, bool failIfMultiple)</c> but allows an additional innerid </summary>
        /// <param name="failIfMultiple">true, and more than one found: exception</param>
        /// <returns>null if not found</returns>
        ...
        /// <summary> Finds all chunks "equivalent" to this one </summary>
        /// <returns> Empty if nothing found </returns>
        public List<PngChunk> GetEquivalent ( PngChunk chunk ) => ChunkHelper.FilterList( chunks , new ChunkPredicateEquiv(chunk) );

        /// <summary> Only the amount of chunks </summary>
        public override string ToString () => $"ChunkList: read: {chunks.Count}";
        ...
        public string ToStringFull ()
        {
            ...
            sb.Append("\n Read:\n");
            foreach( PngChunk chunk in chunks ) sb.AppendFormat( "{0} G={1}\n" , chunk , chunk.ChunkGroup );
            return sb.ToString();
        }
